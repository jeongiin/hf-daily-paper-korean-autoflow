            ...
            messages=[
                {
                    ...
                    "content": """
                    당신은 AI분야의 한국인 전문가입니다. 
                    번역을 할 때 정확성과 전문성을 유지하고 학술 용어를 사용해주세요.
                    기술 용어는 영어로 그대로 작성하고 다른 문장은 간결하고 자연스러운 한국어 문장으로 작성해주세요.
                    """
                },
                {
    ...
    try:
        ...
        for paper_str in tqdm(papers_data, desc="논문 처리 중"):
            try:
                ...
                "method":["웹 애플리케이션을 만들어 text-to-image 사용자들이 이미지를 생성하고 선호도를 기록하도록 함\nTo address this issue, we create a web app that enables text-to-image users to generate images and specify their preferences.", "사용자 데이터를 기반으로 Pick-a-Pic 이라는 대규모 공개 데이터세트를 생성함\nUsing this web app we build Pick-a-Pic, a large, open dataset of text-to-image prompts and real users' preferences over generated images.", "Pick-a-Pic 데이터를 활용해 CLIP 기반 평가 함수인 PickScore를 학습시키고, 인간 선호 예측에서 뛰어난 성능을 확인함.\nWe leverage this dataset to train a CLIP-based scoring function, PickScore, which exhibits superhuman performance on the task of predicting human preferences.","PickScore를 사용해 모델 평가를 수행하고, 기존 자동 평가 지표보다 인간 랭킹과 더 잘 일치하는 결과를 관찰함.\nThen, we test PickScore's ability to perform model evaluation and observe that it correlates better with human rankings than other automatic evaluation metrics.",
